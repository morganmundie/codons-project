...
std::string GetLongestOpenReadingFrame(const std::string & DNA_sequence){
    std::vector<std::vector<string>> frames = GetReadingFramesAsCodons(DNA_sequence);
    std::vector<string> open_reading_frames;

    for (std::vector<string> frame : frames) {
        string amino_acid_sequence = Translate(frame);
        for (int i = 0; i < static_cast<int>(amino_acid_sequence.size()); ++i) {
            if (amino_acid_sequence[i] != 'M') {
                continue;
            }
            for (int j = i; j < static_cast<int>(amino_acid_sequence.size()); ++j) {
                if (amino_acid_sequence[j] == '*') {
                    open_reading_frames.push_back(
                        amino_acid_sequence.substr(i, j - i + 1));
                    break;
                }
            }
        }
    ...
    string longest;
    for (string orf : open_reading_frames) {
